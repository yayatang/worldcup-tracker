          ...
          <div className="flex gap-4 overflow-x-hidden">
            {[8, 4, 2, 1].map((n, i) => (
              <div key={i} className="flex flex-col gap-2 flex-shrink-0">
                <div className="h-3 w-20 bg-neutral-800 rounded animate-pulse mb-1 mx-auto" />
                {Array.from({ length: n }).map((_, j) => (
                  <div
                    key={j}
                    className="w-44 bg-neutral-900 border border-neutral-800 rounded-lg animate-pulse"
                    style={{ height: 72 }}
                  />
                ))}
              </div>
